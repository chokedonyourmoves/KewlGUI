...
-- ----------------------------------------------------------------------
-- ESP Drawing Objects & Functions (defined BEFORE UI)
-- ----------------------------------------------------------------------

-- FOV Circle (must exist before UI callbacks use it)
...
function UpdateEsp()
    ...
    for player, esp in pairs(EspObjects) do
        ...
        if character and humanoid and humanoid.Health > 0 and head then
            ...
            local distance = (LocalPlayer.Character and LocalPlayer.Character:GetPivot().Position - character:GetPivot().Position).Magnitude or 0

            if headOnScreen and distance <= Settings.MaxDistance then
                -- Team check: skip teammates (NO "continue" – illegal in Lua!)
...
-- ----------------------------------------------------------------------
-- UI BUILDING (now all required functions & drawings exist)
-- ----------------------------------------------------------------------
...
local TextSizeSlider = SettingsTab:CreateSlider({
    ...
    Callback = function(Value)
        ...
        -- Update existing ESP text sizes
        for _, esp in pairs(EspObjects) do
            if esp.Name then esp.Name.Size = Value end
            if esp.Health then esp.Health.Size = Value end
            if esp.Distance then esp.Distance.Size = Value - 2 end
        end
    end,
})
...
SettingsTab:CreateSection("Color Settings")
...
SettingsTab:CreateSection("Team Settings")
...
if IsMobile then
    ...
    MobileTab:CreateSection("Mobile Controls")
    MobileTab:CreateLabel({ Text = "Mobile Gesture Controls:", TextXAlignment = "Left" })
    MobileTab:CreateLabel({ Text = "• Double Tap: Toggle ESP", TextXAlignment = "Left" })
    MobileTab:CreateLabel({ Text = "• Triple Tap: Toggle GUI", TextXAlignment = "Left" })
...
InfoTab:CreateLabel({ Text = "Version: 2.0.0 (Rayfield Edition)", TextXAlignment = "Left" })
InfoTab:CreateLabel({ Text = "Features:", TextXAlignment = "Left" })
InfoTab:CreateLabel({ Text = "• Full ESP System with Boxes, Tracers", TextXAlignment = "Left" })
InfoTab:CreateLabel({ Text = "• Customizable FOV Circle", TextXAlignment = "Left" })
InfoTab:CreateLabel({ Text = "• Mobile & PC Support", TextXAlignment = "Left" })
InfoTab:CreateLabel({ Text = "• Team Check Filter", TextXAlignment = "Left" })
InfoTab:CreateDivider()
InfoTab:CreateLabel({ Text = "Controls:", TextXAlignment = "Left" })
InfoTab:CreateLabel({ Text = "PC: Press 'K' to toggle GUI", TextXAlignment = "Left" })
InfoTab:CreateLabel({ Text = "Mobile: Use gestures or Mobile tab", TextXAlignment = "Left" })
InfoTab:CreateDivider()
...
-- Create ESP for already loaded players
for _, player in ipairs(Players:GetPlayers()) do
    if player ~= LocalPlayer then
        CreateEsp(player)
    end
...
table.insert(Connections, Camera:GetPropertyChangedSignal("ViewportSize"):Connect(UpdateFOV))

-- ----------------------------------------------------------------------
-- Initial state: ESP off, FOV visibility as per Settings
-- ----------------------------------------------------------------------
ToggleESP(Settings.Enabled)
FOVCircle.Visible = Settings.ShowFOV
...
return {
    ...
    ShowGUI = function() Rayfield:SetVisibility(true) end,
    HideGUI = function() Rayfield:SetVisibility(false) end,
    Destroy = DestroyAll
}
